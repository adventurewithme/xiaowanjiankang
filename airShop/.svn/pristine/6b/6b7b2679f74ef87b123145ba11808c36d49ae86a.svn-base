	...
	<view class="container">
		...
		<view class="seckill-section m-t" v-if="seckillList != null && seckillList.length > 0">
			<scroll-view class="floor-list" scroll-x="true">
				<view class="floor-listdiv" v-for="(item, index) in seckillList" :key="index"  @click="navToDetailPage({ link: '/pages/product/product?id=' + item.id })">
					<view class="floor-list-left">
						...
						<view class="left-time">
							<text class="time hour">{{seckillListtimes[index][0]}}</text>
							<text class="dots">:</text>
							<text class="time min">{{seckillListtimes[index][1]}}</text>
							<text class="dots">:</text>
							<text class="time second">{{seckillListtimes[index][2]}}</text>
						</view>
