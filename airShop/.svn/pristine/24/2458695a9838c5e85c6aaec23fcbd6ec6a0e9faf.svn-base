					...
					>
						...
						<scroll-view v-if="item.goodsList.length > 1" class="goods-box" scroll-x>
							<view
								...
								class="goods-item"
								@click="toorderdetail(goodsItem.id)"
							>	
						...
						>
							...
							<view class="right">
								...
								<text class="attr-box"><text v-for="(i) in goodsItem.attr">{{i+" "}}</text>  x {{goodsItem.number}}</text>
								<text class="price">{{goodsItem.price}}</text>
								<text class="attr-box">取货地址:{{item.address}}</text>
								<text class="attr-box">取货时间:{{item.fetchTime}}</text>
							</view>
						...
						<view class="action-box b-t" >
							<button class="action-btn" @click="cancelOrder(item)" v-if="item.state == 50">取消订单</button>
...
<script>
	...
	export default {
		...
		methods: {
			// 
			toorderdetail(id){
				uni.navigateTo({url: '/pages/submitam/orderdetail?goodsid='+id})
			},
			//获取订单列表
			loadData(source){
				//这里是将订单挂载到tab列表下
			...
			orderDetail(id){
				uni.navigateTo({
					url: `/pages/submitam/orderdetail?id=`+id
				})
			},
		}
	}
