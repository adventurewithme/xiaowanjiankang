...
function queryParams() {
    return {
        type: "owner", sort: "updated", direction: "desc", per_page: 100, page: 1
    }
}
function buildTable(e, t, o) {
    ...
    e.bootstrapTable("destroy").bootstrapTable({
        columns: a, data: c, iconSize: "outline", icons: {
...
!function (e, t, o) {
    ...
    !function () {
        ...
        //         e = res
                // o("#exampleTableFromData1").bootstrapTable({ data: e, height: "250" })
                // o("#exampleTableFromData2").bootstrapTable({ data: e, height: "250" })
            // }
        // });
        // console.log(e)
        // var e = [
...
$('.btn-primary').click(function () {
    // var l = $("#exampleTableFromData tbody tr:last-child")[0].dataset.index;
    // l = parseInt(l) + 1
    // var node = $('<tr data-index=' + l + ' style="min-height: 30px;"><td style>序号</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>')
    // $("#exampleTableFromData").append(node)
    tablelength++;
    var pri_title = $('<li role="presentation"><a href="#exampleTableFromData' + tablelength+'">训练'+tablelength+'</a></li>');
    $('.nav-pills').append(pri_title);
    var table = $('<div class="example hide">'+
        '<table class="table" id = "exampleTableFromData' + tablelength+'" data-mobile-responsive="true">' +
        '<thead>'+
            '<tr>'+
                '<th data-field="id">序号'+tablelength+'</th>'+
                '<th data-field="name">动作名称</th>'+
                '<th data-field="video">动作视频名</th>'+
                '<th data-field="size">动作类型</th>'+
                '<th data-field="length">动作长度</th>'+
                '<th data-field="hot">消耗热量</th>'+
                '<th data-field="operation">操作</th>'+
            '</tr>'+
        '</thead>'+
                        '</table >'+
                    '</div >');
        $(".row .col-sm-12").append(table);
});
$.ajax({
    url: '/json/add.json', 
    success: function (res) {
        for(var i=0;i<res.length;i++){
            var node = $('<li><a href="#">'+res[i].name+'</a></li>')
            $('.changedown').append(node);
        }
    }
});
// 切换循环表格
$('.nav-pills').on('click','li',function(e){
    ...
    $('.col-sm-12 .example').addClass('hide');
    $('.col-sm-12 .example:eq('+c_id+')').removeClass('hide');
})