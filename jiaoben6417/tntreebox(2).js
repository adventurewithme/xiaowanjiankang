...
_tnTreebox.prototype = {
		...
		$("#"+this.dom_id+" :checkbox").on('change',function(e){
            ...
            for (var i = 0; i < $('#treebox ul li').length;i++){
                if (value == $('#treebox ul li:eq(' + i + ')').find('label').eq(0).html()){
                    $('#treebox ul li:eq(' + i + ')').not(thisli).toggleClass('selected');
	...
	showChecked:function(){
		...
		$("#"+this.dom_id+" input:checked").not('.hide2 input:checked').each(function(i,e){
			...
			var id = $(this).val();
			var htmlStr="<div>"+text+"<span v="+id+">x</span></div>";
			console.log(htmlStr.indexOf(htmlStr),'info');
			if(html.indexOf(htmlStr) < 0){
				html += "<div>"+text+"<span v="+id+">x</span></div>";
			}
		
			//选中同名运动
			/*$("#"+obj.dom_id).find("li").each(function(){
				if($(this).find("label").eq(0).html()==text){
